import base64, converting, os, threading, time, datetime, json, math, requests, tarfile, urllib, settings, mimetypes, socket, pyzipper, drive_requests

def get_folder():
    return_slug = []
    source_dir_list = os.listdir(settings.SOURCE_FOLDER)
    exclude_list = list(json.load(open(os.path.join(settings.DATA_FOLDER,"options.json"))).get("exclude_folders","").split(","))
    for s in source_dir_list:
        loop_json = {}
        loop_json["slug"] = s
        loop_json["id"] = s
        loop_json["name"] = s
        type = "File" if os.path.isfile(os.path.join(settings.SOURCE_FOLDER, s)) else "Folder"
        loop_json["description"] = f"{type} named {s}"
    ...
    for item in exclude_list:
        if not item in source_dir_list:
            ...
            loop_json["id"] = item
            loop_json["name"] = item
            type = "File" if os.path.isfile(os.path.join(settings.SOURCE_FOLDER, item)) else "Folder"
            loop_json["description"] = f"{type} named {item}"
            return_slug.append(loop_json)
    return return_slug
...
def generate_config():
    ...
    none_list = {"backup_time_of_day","backup_password","exclude_folders","exclude_addons","stop_addons","drive_ipv4","default_drive_client_secret","drive_picker_api_key","supervisor_url","hassio_header","server_project_id","snapshot_time_of_day","snapshot_password"}
    true_list = {"confirm_multiple_deletes","warn_for_low_space","enable_drive_upload"}
    false_list = {"ignore_upgrade_backups","delete_after_upload", "generational_delete_early","ignore_other_backups","delete_before_new_backup","specify_backup_folder","call_backup_snapshot","disable_watchdog_when_stopping","use_ssl","require_login","expose_extra_server","verbose","send_error_reports","drive_experimental","ignore_ipv6_addresses","ignore_other_snapshots","ignore_upgrade_snapshots","specify_snapshot_folder","delete_before_new_snapshot"}
    old_list = {"notify_for_stale_snapshots","enable_snapshot_stale_sensor","enable_snapshot_state_sensor","notify_for_stale_backups","enable_backup_stale_sensor","enable_backup_state_sensor",}
    ...
    with open(settings.DATA_FOLDER + 'options.json') as f: # 0.3 sec of 2 sec for 1000
        options_data = json.load(f)
    result["config"] = result["defaults"].copy()
    for item in options_data:
        result["config"][item] = options_data[item]
    folders =  get_folder()
    list_memory = {"maximum_upload_chunk_bytes", "google_drive_page_size", "low_space_threshold", "default_chunk_size"}
    for item in list_memory:
        result["config"][item] = converting.bytes_to_human(result["config"][item]) # 0.2 of 2 for 10000
    ...
    for item in list_time:
        result["config"][item] = converting.seconds_to_human(result["config"][item]) # 0.2 of 2 for 10000
...
def save_config(rx_data):
    ...
    with open(settings.DATA_FOLDER + 'options.json', 'w') as f:
        json.dump(options_data, f, indent=2)

def get_backup_info(file_name, type="hi",valid=True):
    my_tarfile = tarfile.open('./pi.tar')
    person= my_tarfile.extractfile('info.json').read()
